...
use lens::{
    collateral_swap, ICollateralSwapDispatcher, ICollateralSwapDispatcherTrait,
...
fn declare_and_deploy() -> ICollateralSwapDispatcher {
    let contract = declare("collateral_swap").unwrap();
    let (contract_address, _) = contract
        .deploy(@array![
            test_address().into(), 
            VESU_SINGLETON_ADDRESS, 
            JEDISWAP_ROUTER_ADDRESS,
            1  // pool_id, adjust as needed
        ])
        .unwrap();
...
fn setup() -> (ICollateralSwapDispatcher, IERC20Dispatcher, IERC20Dispatcher, u256) {
    ...
    assert(vesu_balance > amount, 'vesu needs more ETH');
...
fn test_direct_swap() {
    ...
    // Create the swap path with fee
    let swap_path = array![
        eth_token.contract_address.into(),
        3000,  // 0.3% fee
        usdc_token.contract_address.into()
    ];
    ...
    dispatcher.swap_collateral(
        ...
        1900000000,  // min USDC out
        3000,        // fee
        Option::None,  // No intermediate token
        Option::None   // No intermediate fee
    );
}
